...
.. attention::
    ...
    本篇内容和 sophon-stream/docs/docs_zh/FAQ 内容一样，本篇与其重复只是为了方便查找。
    原文地址：https://github.com/sophgo/sophon-stream
...
先检查boost库的版本：\ ``dpkg -S /usr/include/boost/version.hpp``
1.71版本可以正常编译运行，较旧的版本可能报如上错误。

如果不便安装该版本boost，可以使用我们提供的\ `docker镜像进行编译 <./HowToMake.md#使用开发镜像编译>`__
...
如果不便安装该版本交叉编译器，可以使用我们提供的\ `docker镜像进行编译 <./HowToMake.md#使用开发镜像编译>`__
...
此种情况下，请参考 `encode
element <../element/multimedia/encode/README.md>`__\ ，开启流服务器后重试。
